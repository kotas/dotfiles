...
# 1.1.1 -- 2009-04-10
#       fix a problem when an item in a search window is selected in Mac OS X 10.5.
# 1.1 -- 2008-03-26
...
on run
	...
	on error
		...
		return POSIX path of (a_location as alias)
	end try
...
on process_for_special_items()
	...
	tell application "Finder"
		...
		if a_name is displayed name of trash then
			...
			return a_location
		else if current view of Finder window 1 is group view then -- in Mac OS X 10.5 search window is not group view
			return my location_for_selection()
		else -- Search window or Network or Unknown
			try
				return my location_for_selection()
			on error
				return missing value
